...
for type in network_types:
    for let in lottery:
        for target in targets:
            ...
            base_out_dir = os.path.join(current_directory, 'out', 'lottery', target, f"{kwargs['network_type']}-{kwargs['lr']}-{kwargs['epochs']}-{kwargs['batch_size']}-f{let}")
            ...
            if target == "FIRE":
                ...
                for i in range(14+49, 14+49+70):  #14+49+70
                    ...
                    if let < 2:
                        ...
                        model, best_loss = general.select_best_initialization(moving_image, fixed_image, kwargs, num_trials=let, plot=False)
