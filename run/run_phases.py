...
losses = ["mse", "l1", "ncc", "smoothl1"]
learning_rates = [0.00001, 0.0001]   
batch_sizes = [50000]
network_types = ["SIREN", "MLP"] 
lottery = 1
phases = [1, 2, 3]
...
for type in network_types:
    for ph in phases:
        for target in targets:
            ...
            kwargs["lr"] = 0.0001
            kwargs["batch_size"] = 25000  #max 1.500.000 ... 16 -> 256 -> 65536
            kwargs["phases"] = ph # 1 is normal, 2 does fiest half with sqrt(batch_size), second half with batch_size, etc...
            ...
            base_out_dir = os.path.join(current_directory, 'out', 'phases', target, f"{kwargs['network_type']}-{kwargs['lr']}-{kwargs['epochs']}-{kwargs['batch_size']}-f{ph}")
