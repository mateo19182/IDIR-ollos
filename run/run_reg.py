...
for type in network_types:
    for reg in regs:
        for target in targets:
            ...
            kwargs["alpha_hyper"] = reg[0]  #0.25
            kwargs["jacobian_regularization"] = True
            kwargs["alpha_jacobian"] = reg[1]  #0.05 default
            kwargs["bending_regularization"] = True
            kwargs["alpha_bending"] = reg[2]   #10.0
                    
            kwargs["save_checkpoints"] = False

            data_dir = os.path.join(current_directory, 'data/', target)
            base_out_dir = os.path.join(current_directory, 'out', 'reg', target, f"{kwargs['network_type']}-{kwargs['lr']}-{kwargs['epochs']}-{kwargs['batch_size']}_{reg[0]}-{reg[1]}-{reg[2]}")
